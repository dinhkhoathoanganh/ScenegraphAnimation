...
public abstract class AbstractNode implements INode
{
    ...
    /**
     * All lights contained in this node.
     */
    ...
    protected List<Light> collectLights(final Matrix4f transformation) {
        List<Light> adjustedLights = new ArrayList<>();
        for (Light light : this.lights) {
            Light adjustedLight = new Light(light);
            Vector4f pos = adjustedLight.getPosition();
            Matrix4f lightTransform = new Matrix4f(transformation);
            pos = lightTransform.transform(pos);
            adjustedLight.setPosition(pos);
            Vector4f newSpotDir = adjustedLight.getSpotDirection().mul(transformation);
            adjustedLight.setSpotDirection(newSpotDir.x, newSpotDir.y, newSpotDir.z);
            adjustedLights.add(adjustedLight);
        }

        return adjustedLights;
    }
}
