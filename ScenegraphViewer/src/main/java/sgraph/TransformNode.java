...
public class TransformNode extends AbstractNode
{
    ...
    @Override
    public List<Light> getLightsInViewSystem(Stack<Matrix4f> transformation) {
        List<Light> adjustedLights = new ArrayList<>();

        transformation.push(new Matrix4f(transformation.peek()));
        transformation.peek().mul(animation_transform)
                .mul(transform);
        adjustedLights.addAll(collectLights(transformation.peek()));

        if (child != null) {
            adjustedLights.addAll(child.getLightsInViewSystem(transformation));
        }
        transformation.pop();
